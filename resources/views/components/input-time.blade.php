    ...
    <flux:field>
        <div class="space-y-2">
            <label for="{{ $wire }}" class="block text-sm font-medium text-zinc-700 dark:text-zinc-300">
                {{ $texto }}
                @if($requerido)
                    <flux:badge class="!text-zinc-800/70 !ml-0.5" size="sm" color="zinc" inset="top bottom">Obligatorio</flux:badge>
                @endif
            </label>
            
            <input
            type="time" id="{{ $wire }}" name="{{ $wire }}" wire:model.live="{{ $wire }}" step="60" required
            @disabled($disabled)
