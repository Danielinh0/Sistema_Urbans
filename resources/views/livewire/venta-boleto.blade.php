<div class="space-y-6 pb-10">
    ...
    <div class="bg-white dark:bg-neutral-900 rounded-2xl border border-neutral-200 dark:border-neutral-700 shadow-sm overflow-hidden">
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-400 dark:text-gray-500">
                        {{ \Carbon\Carbon::parse($filtroFecha)->locale('es')->isoFormat('ddd D [de] MMMM, YYYY') }}
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="flex flex-col items-center justify-center py-14 text-center">
            ...
            <p class="text-gray-400 dark:text-gray-600 text-sm mt-1">Prueba con otra fecha o ruta.</p>
        </div>
        @else
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-gray-50 dark:bg-neutral-800/60">
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Salida</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Llegada</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Unidad</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Chofer</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Destino</th>
                        <th class="px-4 py-3 text-center text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Asientos</th>
                        <th class="px-4 py-3 text-right text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Tarifa</th>
                        <th class="px-4 py-3 text-center text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Estado</th>
                        <th class="px-4 py-3 text-right text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Acción</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100 dark:divide-neutral-800">
                    ...
                    <tr wire:key="corrida-{{ $corrida['id'] }}"
                        ...
                        <td class="px-4 py-3 text-center">
                            @if($corrida['lleno'])
                            <flux:badge color="red">Lleno</flux:badge>
                            @elseif($corrida['libres'] <= 3)
                                <flux:badge color="amber">{{ $corrida['libres'] }} Libres</flux:badge>
                                @else
                                <flux:badge color="green">{{ $corrida['libres'] }} Libres</flux:badge>
                                @endif
                        </td>
                        ...
                        <td class="px-4 py-3 text-center">
                            ...
                            @endphp
                            <flux:badge :color="$estadoColor">{{ $corrida['estado'] }}</flux:badge>
                        </td>

                        {{-- Botón acción --}}
                        <td class="px-4 py-3 text-right">
                            @if($corrida['lleno'])
                            <flux:button size="sm" variant="ghost" disabled class="opacity-40">Lleno</flux:button>
                            @elseif($corridaId === $corrida['id'])
                            <flux:button size="sm" variant="primary"
                                wire:click="seleccionarCorrida({{ $corrida['id'] }})">
                                ✓ Elegida
                            </flux:button>
                            @else
